package expo.modules.provideoeditor.src.features.render.models

import expo.modules.provideoeditor.src.core.constants.PACKAGE_TAG
import expo.modules.provideoeditor.src.shared.logging.PluginLog as Log
import expo.modules.provideoeditor.src.shared.media.EncodedImage
...
data class VideoClip(
     ...
     *
     * Internal, never parsed from the JS boundary. `chromaKey = null`
    ...
    companion object {
        /** Parses a clip from a render-config map. */
 ...
 *
 * Mirrors the JS `ChromaKey` type and the Swift `ChromaKeyConfig`. The
...
data class RenderConfig(
    ...
    companion object {
        /**
         * Creates a RenderConfig from a raw render-config map handed across the
         * JS boundary by Expo's `AsyncFunction` (a plain `[String: Any]`
         * dictionary in TS, bridged to a `Map<String, Any?>` here).
         *
         * @param args The map containing all render parameters
         * @throws IllegalArgumentException if required videoClips are missing or invalid
         */
        fun fromArguments(args: Map<String, Any?>): RenderConfig {
            // Parse multi-layer composition (layered path).
            @Suppress("UNCHECKED_CAST")
            val compositionRaw = args["composition"] as? Map<String, Any?>
            val composition = compositionRaw?.let { CompositionConfig.fromMap(it) }

            // Parse video clips (single-track path).
            @Suppress("UNCHECKED_CAST")
            val videoClipsRaw = args["videoClips"] as? List<Map<String, Any?>>
            ...
            // Parse image layers
            @Suppress("UNCHECKED_CAST")
            val imageLayersRaw = args["imageLayers"] as? List<Map<String, Any?>>
            ...
            // Parse color filters
            @Suppress("UNCHECKED_CAST")
            val colorFiltersRaw = args["colorFilters"] as? List<Map<String, Any?>>
            ...
            // Parse audio tracks
            @Suppress("UNCHECKED_CAST")
            val audioTracksRaw = args["audioTracks"] as? List<Map<String, Any?>>
            ...
            @Suppress("UNCHECKED_CAST")
            val chromaKeyRaw = args["chromaKey"] as? Map<String, Any?>
            ...
            return RenderConfig(
                ...
                rotateTurns = (args["rotateTurns"] as? Number)?.toInt(),
                flipX = args["flipX"] as? Boolean ?: false,
                flipY = args["flipY"] as? Boolean ?: false,
                cropWidth = (args["cropWidth"] as? Number)?.toInt(),
                cropHeight = (args["cropHeight"] as? Number)?.toInt(),
                cropX = (args["cropX"] as? Number)?.toInt(),
                cropY = (args["cropY"] as? Number)?.toInt(),
                scaleX = (args["scaleX"] as? Number)?.toFloat(),
                scaleY = (args["scaleY"] as? Number)?.toFloat(),
                outputWidth = (args["outputWidth"] as? Number)?.toInt(),
                outputHeight = (args["outputHeight"] as? Number)?.toInt(),
                bitrate = (args["bitrate"] as? Number)?.toInt(),
                maxFrameRate = (args["maxFrameRate"] as? Number)?.toInt(),
                enableAudio = args["enableAudio"] as? Boolean ?: true,
                playbackSpeed = (args["playbackSpeed"] as? Number)?.toFloat(),
                colorFilters = colorFilters,
                audioTracks = audioTracks,
                blur = (args["blur"] as? Number)?.toDouble(),
                chromaKey = ChromaKeyConfig.fromMap(chromaKeyRaw),
                startUs = (args["startUs"] as? Number)?.toLong(),
                endUs = (args["endUs"] as? Number)?.toLong(),
                shouldOptimizeForNetworkUse = args["shouldOptimizeForNetworkUse"] as? Boolean
                    ?: true,
                imageBytesWithCropping = args["imageBytesWithCropping"] as? Boolean ?: false
            )
        }
    }
}
