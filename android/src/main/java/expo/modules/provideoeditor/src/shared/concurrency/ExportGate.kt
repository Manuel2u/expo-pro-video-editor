package expo.modules.provideoeditor.src.shared.concurrency

import java.util.concurrent.atomic.AtomicBoolean

/**
 * Process-wide gate that serializes hardware-encoder-heavy exports across the
 * render pipeline so a second concurrent Media3 [androidx.media3
 * .transformer.Transformer] can't wedge at `progress == 0` while the device's
 * limited [android.media.MediaCodec] encoder pool is exhausted. It mirrors the
 * Darwin `ExportGate`.
 *
 * Unlike a lock this **never blocks a thread**: the pipeline drives its
 ...
 * called back — on the thread that frees the slot, i.e. the main Looper — when a
 * slot is available. The wait therefore happens *before* a job arms its stall
 * watchdog, so queueing never counts as a stall.
 *
 ...
 * `Transformer` (which requires its Looper). This holds throughout the module:
 * every export terminal runs on the main Looper (Media3 listeners, the progress
 * poll loop), and cancellation arrives via the Expo Modules API's `AsyncFunction`
 * handler, which is the main thread. The gate stays framework-agnostic (no
 * `Handler`) so it remains JVM-unit-testable; the main-Looper guarantee is the
