...
///
/// Bytes that arrive from JS are already resident and stay there; a file is
/// handed to ImageIO as a URL, so nothing but the decoded image is held. A
/// stop-motion sequence is the case that makes the difference matter — a few
/// hundred phone photos passed as bytes at once is hundreds of megabytes
/// before any of them is decoded.
///
/// Mirrors the Android `EncodedImage`. Modelling the choice as a sum type
/// rather than two optionals is deliberate: "neither is set" and "both are
/// set" are not states any caller has to think about.
enum EncodedImage: Sendable, Equatable {
  ...
  /// Reads an image source out of a JS-supplied argument map, preferring the
  /// on-disk path a caller sends for a file-backed image over inline bytes.
  ///
  ...
  static func from(
    ...
    guard let data = args[dataKey] as? Data, !data.isEmpty else { return nil }
    return .bytes(data)
  }
