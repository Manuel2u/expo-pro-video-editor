...
internal struct VideoClip: Sendable {
  ...
  ///
  /// Internal, never parsed from the JS boundary. `chromaKey == nil` means
  ...
  /// Parses a clip from a JS-boundary argument map. Used by both the
  /// single-track (`videoClips`) and the layered (`composition`) paths.
